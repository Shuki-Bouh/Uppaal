...
class SoupAB3(Soup):
    def __init__(self):
        ...
        bob_initial_from_warning = Piece("bob_initial_from_warning", lambda c: c.bob == "w" and c.flag_alice, Behaviours.bob_action_initial)  # Je fais le choix d'ajouter une pièce pour montrer qu'un nouveau chemin a été créé
        bob_warning = Piece("bob_warning", lambda c: c.bob == "i", Behaviours.bob_action_warning)
        pieces = [alice_critical, alice_warning, alice_initial, bob_critical, bob_warning, bob_initial, bob_initial_from_warning]
